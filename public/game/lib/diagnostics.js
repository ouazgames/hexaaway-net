var unityDiagnostics = (function () {
  ...
  function openDiagnosticsDiv(GetMetricsInfoFunc) {
    ...
    if (!overlayOpen) {
      ...
      var pageLoadTimeToFrame1Div = document.getElementById(
        "pageLoadTimeToFrame1"
      );
      ...
      var webAssemblyStartupTimeDiv = document.getElementById(
    ...
    function updateWebMetricInfo() {
      ...
      if (
        isNaN(metricsInfo.totalJSHeapSize) &&
        isNaN(metricsInfo.usedJSHeapSize)
      ) {
      ...
      pageLoadTimeDiv.textContent =
        (metricsInfo.pageLoadTime / 1000).toFixed(2) + " sec";
      pageLoadTimeToFrame1Div.textContent =
        (metricsInfo.pageLoadTimeToFrame1 / 1000).toFixed(2) + " sec";
      assetLoadTimeDiv.textContent =
        (metricsInfo.assetLoadTime / 1000).toFixed(2) + " sec";
      webAssemblyStartupTimeDiv.textContent =
        (metricsInfo.webAssemblyStartupTime / 1000).toFixed(2) + " sec";
      codeDownloadTimeDiv.textContent =
        (metricsInfo.codeDownloadTime / 1000).toFixed(2) + " sec";
      gameStartupTimeDiv.textContent =
        (metricsInfo.gameStartupTime / 1000).toFixed(2) + " sec";
    ...
    function formatBytes(bytes) {
      if (bytes >= 1024 * 1024 * 1024)
        return formatNumber(bytes / (1024 * 1024 * 1024)) + "\xa0GB";
      else if (bytes >= 1024 * 1024)
        return formatNumber(bytes / (1024 * 1024)) + "\xa0MB";
    ...
    function GraphPanel(name, fontColor, backgroundColor) {
      ...
      var canvas_width = 250 * pixel_ratio,
        canvas_height = 180 * pixel_ratio,
        textname_xpos = 6 * pixel_ratio,
        textname_ypos = 6 * pixel_ratio,
        graph_area_xpos = 8 * pixel_ratio,
        graph_area_ypos = 30 * pixel_ratio,
        graph_area_width = 234 * pixel_ratio,
        graph_area_height = 140 * pixel_ratio;
      ...
      graph_ctx.font =
        "bold " + 16 * pixel_ratio + "px Helvetica,Arial,sans-serif";
      ...
      return {
        ...
        update: function (value, unitMeasure) {
          ...
          if (unitMeasure) {
            ...
            graph_ctx.fillText(
              formatNumber(value) + " (" + round(min) + "-" + round(max) + ")",
              canvas_width - 10,
              textname_ypos
            );
          }
          ...
          graph_ctx.drawImage(
            graph_canvas,
            graph_area_xpos + pixel_ratio,
            graph_area_ypos,
            graph_area_width - pixel_ratio,
            graph_area_height,
            graph_area_xpos,
            graph_area_ypos,
            graph_area_width - pixel_ratio,
          ...
          graph_ctx.fillRect(
            graph_area_xpos + graph_area_width - pixel_ratio,
            graph_area_ypos,
            pixel_ratio,
            graph_area_height
          );
          ...
          graph_ctx.fillRect(
            ...
            graph_area_ypos,
            pixel_ratio,
            round((1 - value / max) * graph_area_height)
          );
        },
      };
    }
    ...
    function createGraph() {
      ...
      var movingAvgFpsGraphPanel = addGraphPanel(
        new GraphPanel("Average FPS", "#0f0", "#020")
      );
      var usedJsMemGraphPanel = addGraphPanel(
        new GraphPanel("Used JS Mem", "#f60", "#201")
      );
      var usedWasmMemGraphPanel = addGraphPanel(
        new GraphPanel("Used Wasm", "#ff0", "#020")
      );
      ...
      return {
        ...
        plotGraph: function (metrics) {
          ...
          usedWasmMemGraphPanel.update(
            formattedBytesInfo.bytesValue,
            formattedBytesInfo.unitMeasure
          );
          if (!isNaN(metrics.usedJSHeapSize)) {
            formattedBytesInfo = formatBytesInfo(metrics.usedJSHeapSize);
            usedJsMemGraphPanel.update(
              formattedBytesInfo.bytesValue,
              formattedBytesInfo.unitMeasure
            );
          }
