class AspectRatio {
  ...
  recalculateAspectRatio() {
    ...
    if (containerWidth / containerHeight > this.aspectRatio) {
      this.gameCanvas.style.width =
        Math.floor(containerHeight * this.aspectRatio) + "px";
      this.gameCanvas.style.height = "100%";
    } else {
      this.gameCanvas.style.width = "100%";
      this.gameCanvas.style.height =
        Math.floor(containerWidth / this.aspectRatio) + "px";
    }
    this.centerAlignCanvas();
  }
  ...
  update() {
    ...
    if (window.innerWidth - this.rightOffset > window.innerHeight) {
      // Landscape mode.
      this.unityContainer.style.width =
        window.innerWidth - this.rightOffset + "px";
      ...
      this.unityContainer.style.height =
        window.innerHeight - this.bottomOffset + "px";
